...
variable_label(npk)
```
The return value of `variable_label()` is a named list, where each list element corresponds to a column of the data frame.

To assign labels to the columns of a data frame, the right-hand side of the assignment has to be a **named** list or vector.
...
For instance, consider these two data frames:
...
If you try to combine two vectors that have different labels, only  the label of the first vector is retained --
in addition, you will receive a meaningful warning message.
